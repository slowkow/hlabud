...
`a$sequences` has amino acid sequence alignments in a matrix with one column:
...
colnames(d) <- str_replace_all(colnames(d), "\\*", "u")
colnames(d) <- str_replace_all(colnames(d), "\\.", "i")
...
There are many possibilities for analysis of a one-hot encoding of HLA-`r my_gene` alleles from IMGTHLA release `r my_release`.
...
We can highlight which alleles have amino acid Asp (D) at position 57:

```{r umap-p57_D, cache=TRUE, echo = FALSE, fig.dpi = 300, fig.width = 9, fig.height = 6, message=FALSE, warning=FALSE}
...
```{r umap-p57, cache=TRUE, echo = FALSE, fig.dpi = 300, fig.width = 9, fig.height = 6, message=FALSE, warning=FALSE}
