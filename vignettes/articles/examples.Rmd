...
If two HLA alleles have a similar amino acid sequence, then they are more likely to present similar antigens than two alleles with different amino acid sequences.

On the other hand, we might expect that an individual with two highly dissimilar alleles might be more likely to present a greater number of different antigens, resulting in what is called a *divergent allele advantage* (https://doi.org/10.1007/BF02918202).

We can compute the divergence for a set of individuals like this:
...
By default, this function uses the amino acid distance matrix from Granthan 1974 (https://doi.org/10.1126/science.185.4154.862).
